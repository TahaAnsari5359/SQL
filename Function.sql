...
SELECT name,LENGTH(name) as name_length FROM users WHERE id=5;

SELECT name, LOWER(name) AS name_lower FROM users;
SELECT name, UPPER(name) AS name_upper FROM users;
SELECT name, LOWER(CONCAT(name,777)) AS concatinated_as FROM users;

-- TIME
SELECT name,NOW() as time FROM users;
SELECT name,YEAR(NOW()) as time FROM users;
SELECT name,YEAR(date_of_birth) as DOY FROM users;
SELECT name,DAY(now()) as day FROM users;
